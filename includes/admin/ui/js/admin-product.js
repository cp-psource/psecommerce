jQuery(document).ready(function($) {
    ...
    $('input.variation_values').on('keyup', function(e) {
    ...
    /* $( '.mp_variations_select' ).on( 'change', function() {
        ...
        if ($(this).val() == '-1') {
            ...
            if (variation_tags !== "") {
                ...
                $variation_tags_textarea.textext()[0].input().bind('getSuggestions', function(e, data) {
                    ...
                    $(this).trigger(
                        'setSuggestions', { result: textext.itemManager().filter(suggestions, query) }
                    );
                });
            }
        }
...
jQuery(document).ready(function($) {
    ...
    $('.mp_variations_table_box [name="selected_variation[]"]').on('keydown', function(e) {
        if (e.keyCode == 9) {
            e.preventDefault();
