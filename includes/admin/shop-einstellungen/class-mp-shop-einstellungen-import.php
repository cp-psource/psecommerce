...
class MP_Shop_Einstellungen_Import {
	...
	public static function process_form() {
		if ( ! empty( $_POST['mp-store-exporter'] ) ) { // Input var okay.
			...
			if ( ! empty( $_POST['mp-store-import'] ) ) { // Input var okay.
				...
				if ( ! empty( $_POST['mp-shop-einstellungen-text'] ) ) { // Input var okay.
					global $wpdb;
					$settings = base64_decode( $_POST['mp-shop-einstellungen-text'] );
		...
		if ( empty( $action ) ) {
			if ( is_main_site() ) {
				...
				$action = sprintf(
					...
					/* translators: %s: Importer name */
					esc_attr( sprintf( __( 'Installiere %s', 'mp' ), $importer_name ) ),
					__( 'Installiere Jetzt', 'mp' )
				);
			} else {
				$action = sprintf(
					/* translators: URL to wp-admin/import.php */
					__( 'Dieser Importer ist nicht installiert. Bitte installiere Importeure auf <a href="%s">der Hauptseite</a>.', 'mp' ),
	...
	public function display_settings() {
		...
		<form method="post" id="mp-export-form">
			...
			<h2><?php esc_html_e( 'Import/Export-Einstellungen', 'mp' ); ?></h2>
			<p>
				<?php esc_html_e( 'Verwende den folgenden Text, um in eine neue Installation zu exportieren. Oder füge die neue Konfiguration zum Importieren ein.', 'mp' ); ?>
			</p>
			<textarea title="mp-shop-einstellungen-text" name="mp-shop-einstellungen-text" cols="100" rows="10"><?php echo esc_textarea( $options ); ?></textarea><br>

			<input type="submit" class="button button-primary" name="mp-store-import" id="mp-store-import" value="<?php esc_attr_e( 'Konfiguration importieren', 'mp' ); ?>">
