...
class MP_Ajax {
			...
			<div class="mp_popup_content">
				...
				<form name="variation_popup" id="variation_popup">
					...
					<div class="mp-product-field-75 mp-variation-field mp-product-field-last">
						<div class="psource-field-label"><?php _e( 'SKU', 'mp' ); ?> <span class="mp_meta_small_desc"><?php _e( '(Produktnummer)', 'mp' ); ?></span></div>
						<input type="text" name="sku" class="mp-product-field-98 mp-blank-bg" placeholder="<?php esc_attr_e( 'Produktnummer eingeben', 'mp' ); ?>" value="<?php echo esc_attr( MP_Product::get_variation_meta( $variation_id, 'sku' ) ); ?>">

						<div class="psource-field-label"><?php _e( 'Preis', 'mp' ); ?><span class="required">*</span></div>
						<input type="text" name="regular_price" id="regular_price" class="mp-product-field-98 mp-blank-bg mp-numeric mp-required" placeholder="<?php esc_attr_e( 'Preis angeben', 'mp' ); ?>" value="<?php echo esc_attr( MP_Product::get_variation_meta( $variation_id, 'regular_price' ) ); ?>">
					</div>
					...
					<?php if ( $product_type == 'external' ) {//show these fields only for External URL Products ?>
						<div class="mp-product-field-100 mp-variation-field">
							...
							<input type="text" name="external_url" id="external_url" class="mp-required" placeholder="<?php esc_attr_e( 'https://', 'mp' ); ?>" value="<?php echo esc_attr( MP_Product::get_variation_meta( $variation_id, 'external_url' ) ); ?>">
						</div>
					<?php } ?>
					...
					<?php if ( $product_type == 'physical' ) {//show these fields only for Physical Products ?>
						<div class="fieldset_check">
							<label>
								...
								<span><?php _e( 'Begrenze die Anzahl der Produkte pro Bestellung', 'mp' ); ?></span>
							</label>
							<fieldset id="fieldset_has_per_order_limit" class="has_area">
								...
								<input type="text" name="per_order_limit" id="per_order_limit" class="mp-product-field-98 mp-numeric" placeholder="<?php esc_attr_e( 'Unlimitiert', 'mp' ); ?>" value="<?php echo esc_attr( MP_Product::get_variation_meta( $variation_id, 'per_order_limit' ) ); ?>">
							</fieldset>
						</div>
					<?php } ?>
					...
					<div class="fieldset_check">
						...
						<label>
							...
							<span><?php _e( 'Setze ein Verkaufsangebot an', 'mp' ); ?></span>
						</label>
						<fieldset id="fieldset_has_sale" class="has_area">
							<?php _e( 'Preis', 'mp' ); ?><span class="required">*</span><input placeholder="<?php esc_attr_e( 'Angebotspreis eingeben', 'mp' ); ?>" type="text" class="mp-numeric mp-required" name="sale_price[amount]" value="<?php echo esc_attr( MP_Product::get_variation_meta( $variation_id, 'sale_price_amount' ) ); ?>"><br>
					...
					<?php if ( $product_type == 'physical' || $product_type == 'digital' ) {//show these fields only for Physical and Digital Products  ?>
						<div class="fieldset_check">
							...
							<label>
								...
								<span><?php _e( 'Steuern (Sondertarif)', 'mp' ); ?></span>
							</label>
							<fieldset id="fieldset_charge_tax" class="has_area">
								<div class="psource-field-desc"><?php _e( 'Wenn Du möchtest, dass dieses Produkt einen speziellen Steuersatz verwendet, gib diesen hier ein. Wenn Du das Symbol "%" weglässt, wird der Preis als fester Betrag für jedes dieser Produkte im Warenkorb des Benutzers berechnet.', 'mp' ); ?></div>
					...
					<?php if ( $product_type == 'physical' ) {//show these fields only for Physical and Digital Products  ?>
						<div class="fieldset_check">
							...
							<label>
								...
								<span><?php _e( 'Versandkosten berechnen', 'mp' ); ?></span>
							</label>
							<fieldset id="fieldset_has_sale" class="has_area">
								<?php if ( $product_type == 'physical' ) {//show these fields only for Physical Products  ?>
					...
					<?php if ( $product_type == 'physical' || $product_type == 'digital' ) {//show these fields only for Physical and Digital Products    ?>
						<div class="fieldset_check">
							...
							<label>
								...
								<span><?php _e( 'Produktbestand verfolgen', 'mp' ); ?></span>
							</label>
							<fieldset id="fieldset_has_sale" class="has_area">
								...
								<input name="inventory[out_of_stock_purchase]" type="checkbox" <?php checked( 1, MP_Product::get_variation_meta( $variation_id, 'inv_out_of_stock_purchase' ) ); ?> value="1"><?php _e( 'Ermögliche den Kauf dieses Produktes, auch wenn es nicht vorrätig ist', 'mp' ); ?><br>
							</fieldset>
						</div>
					...
					<div class="fieldset_check">
						...
						<label>
							...
							<span><?php _e( 'Zusätzlicher Inhalt / Informationen zu dieser Variante', 'mp' ); ?></span>
						</label>
	...
	public function ajax_login() {
		...
		$error_message = __( 'Hoppla! Du hast einen ungültigen Benutzernamen/eine ungültige E-Mail-Adresse und/oder ein ungültiges Passwort eingegeben.', 'mp' );
	...
	public function lookup_order() {
		...
		wp_send_json_error( array( 'error_message' => __( 'Ups ... wir konnten keine Bestellungen anhand dieser ID finden. Bitte überprüfe Deine Bestellnummer und versuche es erneut.', 'mp' ),
		) );
	}
	...
	public function update_states_dropdown() {
		...
		if ( $country = mp_get_post_value( 'country' ) ) {
			...
			if ( $_states ) {
				$states		 = '<option value="">' . __( 'Wähle eins', 'mp' ) . '</option>';
