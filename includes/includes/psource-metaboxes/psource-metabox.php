<?php
if ( ! defined( 'PSOURCE_METABOX_DIR' ) ) {
	define( 'PSOURCE_METABOX_DIR', plugin_dir_path( __FILE__ ) );
}

if ( ! defined( 'PSOURCE_METABOX_URL' ) ) {
	define( 'PSOURCE_METABOX_URL', plugin_dir_url( __FILE__ ) );
}

if ( ! defined( 'PSOURCE_METABOX_VERSION' ) ) {
	define( 'PSOURCE_METABOX_VERSION', '1.0' );
}

PSOURCE_Metabox::load_fields();
require_once PSOURCE_Metabox::class_dir( 'api.php' );

/**
 * Stores the field scripts that have been printed (so we don't print them more than once)
 *12.3.20 alles fein, ev. Übersetzungen nachchecken, DN
 * @global array
 * @name $printed_field_scripts
 */
$GLOBALS['psource_metaboxes_printed_field_scripts'] = array();

// Save the state of a given metabox
add_action( 'wp_ajax_psource_metabox_save_state', array( 'PSOURCE_Metabox', 'ajax_save_state' ) );
// Add metaboxes below title
add_action( 'edit_form_after_title', array( 'PSOURCE_Metabox', 'add_metaboxes_below_title' ) );

class PSOURCE_Metabox {
	...
	public function __construct( $args = array() ) {
		...
		$this->args = array_replace_recursive( array(
			...
			'ajax_save'          => false,
			'submit_button_text' => __( 'Speichern & Übernehmen', 'psource_metaboxes' ),
		), $args );

		$this->args = apply_filters( 'psource_metabox/init_args', $this->args );

		$this->nonce_action = 'psource_metabox_' . str_replace( '-', '_', $this->args['id'] ) . '_save_fields';
		...
		if ( $this->args['ajax_save'] ) {
			add_action( 'wp_ajax_psource_fields_save', array( &$this, 'ajax_save_process' ) );
					...
					$('body').on('change', '#<?php echo $id ?>', function () {
						...
						$.ajax({
							...
							success: function (data) {
								$(document).trigger('psource_fields_saved_field_' + name, data);
							}
						})
					});
				})
		...
		foreach ( $this->fields as $field ) {
			if ( $field->get_name() == $name ) {
				...
				if ( $is_setting_page ) {
					...
					do_action( 'psource_metabox/before_save_fields', $this );
					do_action( 'psource_metabox/before_save_fields/' . $this->args['id'], $this );
	...
	public function is_closed() {
		$settings = ( is_network_admin() ) ? get_site_option( 'psource_metabox_states', array() ) : get_option( 'psource_metabox_states', array() );
	...
	public function init_conditional_logic() {
		...
		$this->args['class'] .= ' psource-metabox-has-conditional';
	}
	...
	public function admin_notices() {
		if ( isset( $_GET['psource_metabox_settings_saved'] ) ) {
			echo '<div class="updated"><p>' . __( 'Einstellungen gespeichert und übernommen!', 'psource_metaboxes' ) . '</p></div>';
		}

		if ( isset( $_GET['psource_metabox_settings_failed'] ) ) {
			echo '<div class="error"><p>' . __( 'Einige Einstellungen konnten nicht übernommen werden, bitte versuch es erneut.', 'psource_metaboxes' ) . '</p></div>';
		}
	}
	...
	public function add_meta_box_classes( $classes ) {
		return array_merge( $classes, array( 'psource-postbox' ) );
	}
	...
	public function admin_enqueue_scripts() {
		...
		if ( $page == 'shop-einstellungen'
			...
			|| $page == 'shop-einstellungen-import'
			|| $page == 'shop-einstellungen-addons'
			|| $page == 'network-shop-einstellungen'
			...
			|| ( isset( $screenpage->post_type ) && ( $screenpage->post_type == MP_Product::get_post_type() || $screenpage->post_type == "mp_order" || $screenpage->post_type == "mp_coupon") ) )  {
				...
				wp_enqueue_script( 'psource-metaboxes-admin', $this->class_url( 'ui/js/admin.js' ), array(
					'jquery-validate-methods',
					'jquery-ui-position',
					'jquery-effects-highlight'
				), PSOURCE_METABOX_VERSION, true );
		}

		$messages = array(
			'alphanumeric_error_msg' => __( 'Bitte nur Buchstaben und Nummern eingeben', 'psource_metaboxes' ),
			'lessthan_error_msg'     => __( 'Wert muss kleiner sein als {0}', 'psource_metaboxes' ),
		);
		...
		wp_localize_script( 'psource-metaboxes-admin', 'PSOURCE_Metaboxes', array(
			...
			'has'            => __( 'hat', 'mp' ),
			'have'           => __( 'hat', 'mp' ),
			'form_error_msg' => __( 'Hoppla! Das Formular enthält %s1, wobei %s2 unten hervorgehoben wurde!', 'mp' ) . "\n" . __( 'Bitte korrigiere %s1 und versuche dann erneut, das Formular zu senden.', 'mp' ),
		) );
	}
	...
	public function admin_enqueue_styles() {
		wp_enqueue_style( 'psource-metaboxes-admin', $this->class_url( 'ui/css/admin.css' ), array(), PSOURCE_METABOX_VERSION );
	}
	...
	public static function class_dir( $path = '' ) {
		return PSOURCE_METABOX_DIR . ltrim( $path, '/' );
	}
	...
	public static function class_url( $path = '' ) {
		return PSOURCE_METABOX_URL . ltrim( $path, '/' );
	}
	...
	public static function load_fields() {
		require_once self::class_dir( 'class-psource-field.php' );
	...
	public function before_settings_metabox() {
		 ...
		 */
		do_action( 'psource_metabox/before_settings_metabox', $this );
		do_action( 'psource_metabox/before_settings_metabox/' . $this->args['id'], $this );
	}
	...
	public function after_settings_metabox() {
		 ...
		 * @param PSOURCE_Metabox The current metabox
		 */
		do_action( 'psource_metabox/after_settings_metabox', $this );
		do_action( 'psource_metabox/after_settings_metabox/' . $this->args['id'], $this );
	}
	...
	public function render( $post = null ) {
		...
		if ( ! is_null( $this->args['desc'] ) ) :
			?>
			<div class="psource-metabox-desc"><?php echo $this->args['desc']; ?></div>
		<?php endif;
		?>
		<div class="psource-fields clearfix">
			<?php
			foreach ( $this->fields as $field ) :
				$classes = array(
					$field->args['class'] . ' psource-field',
				...
				<div class="<?php echo implode( ' ', $classes ); ?>">
					...
					if ( ! empty( $field->args['label']['text'] ) ) :
						?>
						<div
							class="psource-field-label"><?php echo $field->args['label']['text'] . ( ( ! empty( $field->args['custom']['data-rule-required'] ) ) ? '<span class="required">*</span>' : '' ); ?></div>
						<?php
					endif;

					if ( ! empty( $field->args['desc'] ) ) :
						?>
						<div class="psource-field-desc"><?php echo $field->args['desc']; ?></div>
					<?php endif;
					?>
	...
	public function save_fields( $post_id ) {
		...
		if ( ! is_numeric( $post_id ) ) {
			...
			foreach ( $this->fields as $field ) {
				...
				if ( $field instanceof PSOURCE_Field_Repeater ) {
			 ...
			 */
			do_action( 'psource_metabox/after_settings_metabox_saved', $this );
			do_action( 'psource_metabox/after_settings_metabox_saved/' . $this->args['id'], $this );

			if ( did_action( 'psource_metabox/after_settings_metabox_saved' ) == count( self::$metaboxes ) ) {
				/**
				...
				if ( isset( $quick_setup_step ) && $quick_setup_step == '2' ) {
					...
					wp_redirect( add_query_arg( 'psource_metabox_settings_saved', 1 ), 301 );
				}
				exit;
			}
		} else {
      ...
      // Checking this using cache ends up to not saving on set ups with cache
			...
			if ( wp_cache_get( 'save_fields', 'psource_metaboxes' ) ) {
				return;
			}
			wp_cache_set( 'save_fields', true, 'psource_metaboxes' );
			 ...
			 */
			do_action( 'psource_metabox/save_fields', $post_id );
		}
	}
