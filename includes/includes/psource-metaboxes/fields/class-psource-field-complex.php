...
class PSOURCE_Field_Complex extends PSOURCE_Field {
	...
	public function display( $post_id ) {
		...
		$class		 = 'psource-field-complex-wrap clearfix ' . $this->args[ 'layout' ];
		$atts		 = '';
		...
		<div class="<?php echo $class; ?>"<?php echo $atts; ?>>
			...
			foreach ( $this->subfields as $field ) :
				...
				if ( $this->args[ 'layout' ] == 'columns' ) :
					?>
					<label class="psource-field-complex-label" style="width:<?php echo $label_width; ?>">
						<?php if ( isset( $this->args[ 'custom' ][ 'label_position' ] ) && $this->args[ 'custom' ][ 'label_position' ] == 'up' ) { ?>
							<?php if ( isset( $this->args[ 'custom' ][ 'label_type' ] ) && $this->args[ 'custom' ][ 'label_type' ] == 'standard' ) { ?>
								<div class="psource-field-label"><?php echo $field->args[ 'label' ][ 'text' ] . ((!empty( $field->args[ 'custom' ][ 'data-rule-required' ] ) ) ? '<span class="required">*</span>' : ''); ?></div>
							<?php } else { ?>
				...
				<?php else :
					?>
					<label class="psource-field-complex-label"><?php $field->display( $post_id ); ?><span><?php echo $field->args[ 'label' ][ 'text' ] . ((!empty( $field->args[ 'custom' ][ 'data-rule-required' ] ) ) ? '<span class="required">*</span>' : ''); ?></span></label>
						<?php
						endif;
					endforeach;
					?>
	 ...
	 * @param array $args @see PSOURCE_Field construct
	 */
	public function add_field( $type, $args ) {
		$class = apply_filters( 'psource_field_complex/add_field', 'PSOURCE_Field_' . ucfirst( $type ), $type, $args );
