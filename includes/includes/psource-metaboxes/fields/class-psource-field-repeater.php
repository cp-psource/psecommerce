...
class PSOURCE_Field_Repeater extends PSOURCE_Field {
	...
	public function save_value( $post_id, $meta_key = null, $value = null, $force = false ) {
		 ...
		 */
		$value = apply_filters('psource_field/save_value', $this->sanitize_for_db($value, $post_id), $post_id, $this);
		$value = apply_filters('psource_field/save_value/' . $this->args['name'], $value, $post_id, $this);
	...
	public function get_value( $post_id, $meta_key = null, $raw = false ) {
		 ...
		 * @param object $this Refers to the current field object
		 */
		$value = apply_filters('psource_field/before_get_value', $value, $post_id, $raw, $this);
		$value = apply_filters('psource_field/before_get_value/' . $this->args['name'], $value, $post_id, $raw, $this);
		 ...
		 * @param object $this Refers to the current field object
		 */
		$value = apply_filters('psource_field/get_value', $value, $post_id, $raw, $this);
		$value = apply_filters('psource_field/get_value/' . $this->args['name'], $value, $post_id, $raw, $this);
	...
	public function sort_subfields( $unsorted ) {
		...
		foreach ( $unsorted as $idx => $fields ) {
			foreach ( $this->subfields as $index => $field ) {
				...
				if ( $field instanceof PSOURCE_Field_Complex ) {
	...
	public function display( $post_id ) {
		...
		$class = 'psource-subfield-group-wrap';
		$atts = '';
		...
		if ( $this->args['layout'] == 'table' ) : ?>
			...
			foreach ( $this->subfields as $index => $subfield ) : ?>
						<th><label class="psource-subfield-label <?php echo $subfield->args['label']['class']; ?>"><?php echo $subfield->args['label']['text'] . (( ! empty( $subfield->args['custom']['data-rule-required'] ) ) ? '<span class="required">*</span>' : '') . (( ! empty($subfield->args['desc']) ) ? '<span class="psource-metabox-tooltip dashicons dashicons-editor-help"><span>' . $subfield->args['desc'] . '</span></span>' : ''); ?></label></th>
			<?php
			endforeach; ?>
		...
		foreach ( $data as $outer_index => $row ) :
			foreach ( $this->subfields as $index => $subfield ) :
				...
				switch ( $this->args['layout'] ) :
					case 'rows' :
						if ( $index == 0 ) : ?>
				<div class="psource-subfield-group">
					<div class="psource-subfield-group-index"><span><?php echo ($outer_index + 1); ?></span></div>
						<div class="psource-subfield-group-fields">
						<?php
						endif; ?>
							<div class="psource-subfield <?php echo str_replace('_', '-', strtolower(get_class($subfield))); ?>">
								<div class="psource-subfield-inner clearfix">
										<?php
						if ( ! empty($subfield->args['label']['text']) ) : ?>
										<label class="psource-subfield-label <?php echo $subfield->args['label']['class']; ?>"><?php echo $subfield->args['label']['text'] . (( ! empty( $subfield->args['custom']['data-rule-required'] ) ) ? '<span class="required">*</span>' : '') . (( ! empty($subfield->args['desc']) ) ? '<span class="psource-metabox-tooltip dashicons dashicons-editor-help"><span>' . $subfield->args['desc'] . '</span></span>' : ''); ?></label>
										<?php
						endif; ?>
									<div class="psource-subfield-input"><?php $subfield->display($post_id); ?></div>
								</div>
							</div>
						<?php
						if ( $index == (count($this->subfields) - 1) ) : ?>
							</div>
							<div class="psource-subfield-delete-group-link-wrap"><a class="psource-subfield-delete-group-link" href="javascript:;"></a></div>
						<?php
						endif;
					break;
					...
					default :
						if ( $index == 0 ) : ?>
				<tr class="psource-subfield-group"><td class="psource-subfield-group-index"><span><?php echo ($outer_index + 1); ?></span></td>
					<?php
						endif; ?>
					<td class="psource-subfield">
						<div class="psource-subfield-inner"><?php $subfield->display($post_id); ?></div>
					</td>
					<?php
						if ( $index == (count($this->subfields) - 1) ) : ?>
					<td class="psource-subfield-delete-group"><a class="psource-subfield-delete-group-link" href="javascript:;"></a></td>
					<?php
						endif;
					break;
				endswitch;
		...
		<div class="psource-repeater-field-actions clearfix"><input class="button-primary psource-repeater-field-add alignright" type="button" value="<?php echo $this->args['add_row_label']; ?>" /></div>
		<script type="text/javascript">
		// Clone the subfields BEFORE any events/plugins are able to modify the markup, bind events, etc
		jQuery('.psource-subfields').not('[data-ready-for-cloning]').each(function(){
			var $this = jQuery(this);
			$this.data('toClone', $this.find('.psource-subfield-group:first')).attr('data-ready-for-cloning', 1);
		});
		</script>
		<?php
		$this->after_field();
...
jQuery(document).ready(function($){
	var updateOrdering = function( $elms ){
		$elms.each(function(i){
			var $this = $(this);
			$this.find('.psource-subfield-group-index').find('span').html(i + 1);
	...
	$('.psource-subfields').not( '.no-sorting' ).sortable({
		"axis" : "y",
		"forceHelperSize" : true,
		"forcePlaceholderSize" : true,
		"items" : ".psource-subfield-group",
		"handle" : ".psource-subfield-group-index",
		"placeholder" : "psource-repeater-field-placeholder",
		"start" : function(e, ui) {
			/**
		...
		"stop" : function(e, ui) {
			 ...
			 * @param object
			 */
			$(document).trigger('psource_repeater_field/stop_sort', [ ui.item ]);
		},
	...
	var $links = $('.psource-subfield-delete-group-link');
	if ( $links.length == 1 ) {
		$links.hide();
	}
	...
	$('.psource-subfields').on('click', '.psource-subfield-delete-group-link', function(event){
		if ( confirm('<?php _e('Sicher das Du dies löschen möchtest?', 'psource_metaboxes'); ?>') ) {
			var $this = $(this),
					...
					subfieldGroupCount = $subfieldGroup.closest('.psource-subfields').find('.psource-subfield-group').length,
					didOne = false;

			$subfieldGroup.find('.psource-subfield-inner').hide();
			 ...
			 * @param object
			 */
			$(document).trigger('psource_repeater_field/after_delete_field_group', [ $subfieldGroup ]);
	...
	$('.psource-repeater-field-add').click(function(){
		 ...
		 * @since 1.0
		 */
		$(document).trigger('psource_repeater_field/before_add_field_group');

		var $btn = $(this),
				$subfields = $btn.closest('.psource-field').find('.psource-subfields'),
				$clonedRow = $subfields.data('toClone').clone(),
				didOne = false;

		$clonedRow.find('.psource-subfield-inner').css('display', 'none');
		...
		$clonedRow.find('.psource-subfield-inner').show();
		 ...
		 * @param object group
		 */
		$(document).trigger('psource_repeater_field/after_add_field_group', [ $clonedRow ]);

		$('.psource-subfields').sortable('refresh'); //so the new row is recognized by the sortable plugin
		$('.psource-subfield-delete-group-link').show();
	});
});
</script>
		<?php
	 ...
	 * @since 1.0
	 * @access public
	 * @return PSOURCE_Field
	 */
	public function add_sub_field( $type, $args ) {
		$class = 'PSOURCE_Field_' . ucfirst($type);
