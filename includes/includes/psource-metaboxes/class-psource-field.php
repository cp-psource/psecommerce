...
class PSOURCE_Field {
	 ...
	 *12.3.20 alles fein, ev. noch Übersetzungen, DN
	 * @since 1.0
	 * @access protected
	 * @var mixed
	 */
	...
	public function __construct( $args = array() ) {
		...
		add_action( 'psource_metabox/save_fields', array( &$this, 'save_value' ) );
	...
	public function init_validation() {
		...
		foreach ( $this->args[ 'validation' ] as $key => $val ) {
			...
			if ( $key == 'custom' ) {
				$index													 = max( (int) wp_cache_get( 'custom_validation_index', 'psource_field' ), 0 );
				$this->args[ 'custom' ][ "data-rule-custom-{$index}" ]	 = $val;
				wp_cache_set( 'custom_validation_index', ($index + 1 ), 'psource_field' );
			} else {
	...
	public function init_conditional_logic() {
		...
		$this->args[ 'class' ] .= ' psource-field-has-conditional';
	}
	...
	public function get_post_value( $key, $default = false ) {
		...
		return apply_filters( 'psource_fields/get_post_value_' . implode( '_', $keys ), $value );
	}
	...
	public function set_subfield_id( $id ) {
		$this->subfield_id	 = $id;
		$this->args[ 'id' ]	 = 'psource-field-' . uniqid( true );
	}
	...
	public function format_value( $value, $post_id ) {
		 ...
		 * @param object $this Refers to the current field object
		 */
		$value = apply_filters( 'psource_field/format_value', $value, $this );
		return apply_filters( 'psource_field/format_value/' . $this->args[ 'name' ], $value, $this );
	}
	...
	public function maybe_print_scripts() {
		 ...
		 */
		do_action( 'psource_field/print_scripts', $this );
		do_action( 'psource_field/print_scripts/' . $this->args[ 'name' ], $this );
	...
	public function scripts_printed() {
		global $psource_metaboxes_printed_field_scripts;

		$class = get_class( $this );

		if ( in_array( $class, $psource_metaboxes_printed_field_scripts ) ) {
			return true;
		}

		$psource_metaboxes_printed_field_scripts[] = $class;
	...
	public function display( $post_id ) {
		wp_die( __( 'Du musst die PSOURCE_Field::display() Methode in Deiner Formularfeldklasse verwenden', 'psource_metaboxes' ), E_USER_ERROR );
	}
	...
	public function parse_atts() {
		 ...
		 * @param object $this Refers to the current field object
		 */
		return apply_filters( 'psource_field/parse_atts', $atts, $this );
	}
	...
	public function get_id() {
		 ...
		 * @param object $this Refers to the current field object
		 */
		return apply_filters( 'psource_field/get_id', $this->args[ 'id' ], $this );
	}
	...
	public function before_field() {
		 ...
		 */
		$text	 = apply_filters( 'psource_field/before_field', $this->args[ 'before_field' ], $this );
		$text	 = apply_filters( 'psource_field/before_field/' . $this->args[ 'name' ], $text, $this );
	...
	public function after_field() {
		 ...
		 * @param PSOURCE_Field The current field object.
		 */
		$text	 = apply_filters( 'psource_field/after_field', $this->args[ 'after_field' ], $this );
		$text	 = apply_filters( 'psource_field/after_field/' . $this->args[ 'name' ], $text, $this );
