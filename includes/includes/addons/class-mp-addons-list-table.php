...
class MP_Addons_List_Table extends WP_List_Table {
	...
	function get_data() {
		...
		foreach ( $addons as $addon ) {
			if ( MP_Addons::get_instance()->is_addon_enabled($addon->class) ) {
				$enabled = true;
				$status = '<a class="button mp-enable-disable-addon" title="' . __('Deaktiviere Erweiterung', 'mp') . '" href="#"><span class="mp-addon-status enabled"></span>' . __('Aktiviert', 'mp') . '</a>';
			} else {
				$enabled = false;
				$status = '<a class="button mp-enable-disable-addon" title="' . __('Aktiviere Erweiterung', 'mp') . '" href="#"><span class="mp-addon-status disabled"></span>' . __('Deaktiviert', 'mp') . '</a>';
			}
	...
	function process_bulk_actions() {
		...
		switch ( $this->current_action() ) {
			case 'enable' :
				...
				$notice = sprintf(_n('1 Erweiterung aktiviert', '%s Erweiterungen aktiviert', $count, 'mp'), $count);
			break;

			case 'disable' :
				MP_Addons::get_instance()->disable($ids);
				$notice = sprintf(_n('1 Erweiterung deaktiviert', '%s Erweiterungen deaktiviert', $count, 'mp'), $count);
			break;
		}
	...
	function column_settings( $item ) {
		if ( mp_addons()->is_addon_enabled( $item['class'] ) && $item['has_settings'] ) {
			return '<a href="' . add_query_arg( array( 'page' => 'shop-einstellungen-addons', 'addon' => $item['class'] ), admin_url( 'admin.php' ) ) . '">' . __( 'Zu den Einstellungen', 'mp' ) . '</a>';
		}
