...
class MP_Shipping_Weight_Rate extends MP_Shipping_API {
   ...
   */
    ...
    $this->public_name = __( 'Versandkosten nach Gewicht', 'mp' );
  ...
  public function init_settings_metabox() {
		...
		if ( $layers instanceof PSOURCE_Field ) {
			$layers->add_sub_field( 'text', array(
				'name' => 'minweight',
				'label' => array( 'text' => __( 'Warenkorb Gewicht', 'mp' ) ),
				'desc' => __( 'Wenn das Wagengewicht größer oder gleich diesem Wert ist, werden die Preise aus dieser Zeile zur Versandkostenberechnung verwendet.', 'mp' ),
				'validation' => array(
					'required' => true,
					'number' => true,
					'min' => 0.01,
  ...
  /**
	...
	* return float $price
	*/
	function calculate_shipping( $price, $total, $cart, $address1, $address2, $zip, $city, $state, $country, $selected_option ) {
...
MP_Shipping_API::register_plugin( 'MP_Shipping_Weight_Rate', 'weight_rate', __('Versandkosten nach Gewicht', 'mp') );