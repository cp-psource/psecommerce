...
if ( ! function_exists( 'mp_get_theme_list' ) ) :
	...
	function mp_get_theme_list() {
		$theme_list = array();
		$theme_dirs = array( mp_plugin_dir( 'ui/themes' ),  WP_CONTENT_DIR . '/psecommerce-styles/' );
...
if ( ! function_exists( 'mp_get_network_setting' ) ) :
	...
	function mp_get_network_setting( $key, $default = null ) {
		$settings = wp_cache_get( 'network_settings', 'psecommerce' );
		if ( ! $settings ) {
			$settings = get_site_option( 'mp_network_settings', $default, false );
			wp_cache_set( 'network_settings', $settings, 'psecommerce' );
		}
...
if ( ! function_exists( 'mp_get_store_caps' ) ) :
	...
	function mp_get_store_caps() {
		if ( $store_caps = wp_cache_get( 'store_caps', 'psecommerce' ) ) {
			return $store_caps;
		}
		...
		wp_cache_set( 'store_caps', $store_caps, 'psecommerce' );
