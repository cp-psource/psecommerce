...
class PSOURCE_Field_Variations extends PSOURCE_Field {

	/**
	 * Runs on construct of parent
	 *Checked 12.3.20 Texte ausbaubar, sonst alles fein DN
	 *Variantenerstellung Felder
	 * @since 1.0
	 * @access public
	 * @param array $args {
		...
		if ( $has_variations ) {
			...
			<div class="tablenav top">
				<div class="alignleft actions bulkactions">
					...
					<select id="bulk-action-selector-top" class="variant_bulk_selected">
						<option value="-1" selected="selected"><?php _e( 'Massenaktionen', 'mp' ); ?></option>
						<option value="variant_update_images"><?php _e( 'Bider aktualisieren', 'mp' ); ?></option>
						<option value="variant_update_inventory"><?php _e( 'Inventar aktualisieren', 'mp' ); ?></option>
						<option value="variant_update_prices"><?php _e( 'Preise aktualisieren', 'mp' ); ?></option>
						<option value="variant_delete"><?php _e( 'Variationen löschen', 'mp' ); ?></option>
					</select>
					<input type="button" name="" id="variant_bulk_doaction" class="button action" value="<?php echo esc_attr( __( 'Hinzufügen', 'mp' ) ); ?>">
				</div>

				<div class="variation-right-actions">
					<input type="button" name="" id="variant_add" class="button action button-primary" value="<?php echo esc_attr( __( 'Neue Variationen hinzufügen', 'mp' ) ); ?>">
				</div>
				<br class="clear">
			</div>
						...
						foreach ( $children as $child ) {
							...
							<tr id="post-<?php echo $child->ID; ?>" data-id="<?php echo esc_attr( $child->ID ); ?>" class="hentry <?php echo $style; ?>">
								...
								<td class="field_default_variation">
									<a class="set-default" title="<?php esc_attr_e( 'Als Standardvariante.', 'mp' ) ?>" href="#" data-child-id="<?php echo esc_attr( $child->ID ); ?>" data-post-id="<?php echo esc_attr( $post_id ); ?>"><i class="fa fa-asterisk"></i></a>
								</td>
							</tr>
						<?php } ?>
					</tbody>
			...
			<div class="mp_hidden_content">
				...
				<div id="mp_bulk_inventory">
					<div class="mp_popup_content">
						<label><?php _e( 'Inventar', 'mp' ); ?></label>
						<input type="text" name="mp_bulk_inventory" class="mp_bulk_inventory" value="" placeholder="<?php echo esc_attr( __( 'Verfügbare Menge einfügen. Unbegrenzt leer.', 'mp' ) ); ?>" required />
					</div>
				...
				<div id="mp_bulk_delete">
					<div class="mp_popup_content">
						...
						<p><?php _e( 'Du bist dabei, ausgewählte Varianten zu löschen. <br/> Wenn Du fortfahren möchten, bestätige den Vorgang bitte.', 'mp' ); ?></p>
					</div>
					<div class="mp_popup_controls mp_delete_controls">
						<a href="" class="button button-primary delete-bulk-form"><?php _e( 'TU ES!', 'mp' ); ?></a>
						<a href="" class="preview button cancel"><?php _e( 'NEIN - BLOSS NICHT!', 'mp' ); ?></a>
					</div>
				</div>
								...
								<td width="100%">
									<div class="variation-row">
										<div class="variation-first-col">
											<div class="psource-field-label"><?php _e( 'Variation Name', 'mp' ); ?> <span class="mp_meta_small_desc"><?php _e( '(z. B. Farbe)', 'mp' ); ?></span></div>
											<?php
											$product_taxonomies = MP_Product_Attributes_Admin::get_product_attributes_select( 'product_attributes_categories[]', 'id' );
											?>
											<!--<span class="variation_create_new_title"><?php _e( 'Oder erstelle neue Variation', 'mp' ); ?></span>-->
											<a href="#" class="mp-variation-add-all button button-primary" style="display: none;"><?php _e( 'Addiere alle Werte', 'mp' ); ?></a>
											<input type="text" class="mp-variation-attribute-name" placeholder="<?php esc_attr_e( __( 'Gib den Variationsnamen ein', 'mp' ) ); ?>" name="variation_names[]" />
										</div>
						...
						<tfoot>
							<tr>
								<td width="10%" colspan="4">
									<a class="add mp-add-new-variation button"><?php _e( 'Andere Variation erstellen', 'mp' ); ?></span>
										<a href="" id="mp_make_combinations" class='button button-primary create-variations-button'><?php _e( 'Variationen erstellen', 'mp' ); ?></a>
								</td>
							</tr>
						</tfoot>
					</table>
