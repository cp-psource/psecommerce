...
class PSOURCE_Field_Checkbox extends PSOURCE_Field {
	...
	public function on_creation( $args ) {
		$this->args = array_replace_recursive( array(
			'value'			 => 1,
			'message'		 => __( 'Aktivieren', 'psource_metaboxes' ),
			'orientation'	 => '',
		), $args );
	}
	...
	public function print_scripts() {
		...
		<script type="text/javascript">
			jQuery( document ).ready( function( $ ) {
				...
				$( '.psource-fields' ).on( 'change', ':checkbox', onChange )
				$( '.psource-fields' ).find( ':checkbox' ).each( onChange );
	...
	public function display( $post_id ) {
		...
		<label style="<?php echo ( $this->args[ 'orientation' ] == 'horizontal' ) ? 'width:' . $this->args[ 'width' ] : ''; ?>" class="<?php echo $this->args[ 'label' ][ 'class' ]; ?>" for="<?php echo $this->get_id(); ?>">
			<input type="checkbox" <?php echo $this->parse_atts(); ?> <?php echo apply_filters( 'PSOURCE_Field_Checkbox_checked', checked( $value, $this->args[ 'value' ] ), $this->args[ 'name' ] ); ?> /> <span><?php echo $this->args[ 'message' ]; ?></span></label>
		<?php
		$this->after_field();
	}
